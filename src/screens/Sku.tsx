...
import { skuColumn } from '@/constant/skuColumn';
...
const Sku = () => {
  ...
  const ActionCellRenderer = (props: any) => (
    <div className="flex items-center gap-1 cursor-pointer p-2"   onClick={() => deleteStore(props.data.sku_id)}>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const savedOrderString = sessionStorage.getItem('skus-rowOrder');
      if (savedOrderString) {
        const savedOrder = JSON.parse(savedOrderString);
        
        // Sort data based on saved order
        const orderedData = data.map(item => {
          const storedItem = savedOrder.find((order: any) => order.sku_id === item.sku_id);
  ...
  const deleteStore = async (storeId: string | number) => {
    try {
      const { error } = await supabase.from('skus').delete().eq('sku_id', storeId);
      ...
      // Update local state
      setRowData(rowData.filter(row => row.sku_id !== storeId));
      
      // Update session storage after deletion
      const updatedData = rowData.filter(row => row.sku_id !== storeId);
      const rowOrder = updatedData.map((row, index) => ({ sku_id: row.sku_id, row_order: index + 1 }));
      sessionStorage.setItem('skus-rowOrder', JSON.stringify(rowOrder));
    } catch (error) {
  ...
  const addStore = async () => {
    try {
      const newStore = {
        sku_id: `S${Math.floor(100 + Math.random() * 900)}`, // Generate a random store ID
        label: 'New Store',
        city: 'New City',
        state: 'NC',
      ...
      const { data, error } = await supabase.from('skus').insert([newStore]).select();
    ...
    <div className="container mx-auto p-2">
     ...
     <h1 className="text-2xl font-bold mb-4">SKU Management</h1>
      ...
      {isLoading ? (
        ...
        <DataTable
          tableKey="skus" // Changed from 'key' to 'tableKey' based on our enhanced component
          columns={skuColumn}
          rowData={rowData}
